...
export default {
  ...
  props: {
    selectedState: String, // 从 SearchBox.vue 传递的选择的州名
  },
  watch: {
    selectedState(newVal) {
      this.createMarkerForSelectedState(newVal);
    },
  },
  methods: {
    initMap() {
      this.map = new google.maps.Map(this.$refs.map, {
        center: { lat: 37.0902, lng: -95.7129 }, // 美国的地理中心
        zoom: 4,// 默认缩放级别
      });

      // 根据初始选择的州名创建标记
      if (this.selectedState) {
        this.createMarkerForSelectedState(this.selectedState);
      }
    },
    createMarkerForSelectedState(stateName) {
      const markerPosition = this.stateMarkers[stateName];
